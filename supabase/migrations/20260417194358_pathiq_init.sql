-- Roadmaps table (public, no auth required for v1 core)
...
-- Anyone can read & create roadmaps in v1 (no auth)
...
-- Node progress (keyed by roadmap_id + node_id, anonymous via local session id)
...
create index on public.node_progress(roadmap_id, session_id);
